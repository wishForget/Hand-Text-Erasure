...
class MaskRefineLoss(nn.Layer):
    ...
    def __call__(self, x, y):
        ...
        if self.type == 'tversky':
            ...
            beta = 0.9
            alpha = 1 - beta
            numerator = paddle.sum(x * y)
            denominator = x * y + alpha * (1 - y) * x + beta * y * (1 - x)
            return 1 - numerator / (paddle.sum(denominator) + 1e-7)
        elif self.type == 'l1':
...
class AdversarialLoss(nn.Layer):
    r"""
    Adversarial loss
    ...
    def __init__(self, type='nsgan', target_real_label=1.0, target_fake_label=0.0):
        ...
        self.type = type
        self.register_buffer('real_label', paddle.to_tensor(target_real_label))
        self.register_buffer('fake_label', paddle.to_tensor(target_fake_label))
...
class StyleLoss(nn.Layer):
    ...
    def __call__(self, x, y, mask=None):
        ...
        if type(mask) != type(None):
            mask1 = F.interpolate(mask, size=(input_size/2, input_size/2), mode='nearest')
            mask2 = F.interpolate(mask, size=(input_size/4, input_size/4), mode='nearest')
            mask3 = F.interpolate(mask, size=(input_size/8, input_size/8), mode='nearest')
            mask4 = F.interpolate(mask, size=(input_size/16, input_size/16), mode='nearest')
        else:
            mask1 = 1
            mask2 = 1
            mask3 = 1
...
class PerceptualLoss(nn.Layer):
    ...
    def __call__(self, x, y, mask):
        ...
        if type(mask) != type(None):
            mask1 = mask
            mask2 = F.interpolate(mask, size=(input_size/2, input_size/2), mode='nearest')
            mask3 = F.interpolate(mask, size=(input_size/4, input_size/4), mode='nearest')
            mask4 = F.interpolate(mask, size=(input_size/8, input_size/8), mode='nearest')
            mask5 = F.interpolate(mask, size=(input_size/16, input_size/16), mode='nearest')
        else:
            mask1 = 1
            mask2 = 1
            mask3 = 1
...
class VGG19(paddle.nn.Layer):
    def __init__(self):
        ...
        self.relu1_1 = paddle.nn.Sequential()
        ...
        self.relu3_3 = paddle.nn.Sequential()
        ...
        self.relu5_4 = paddle.nn.Sequential()
        ...
        for x in range(16, 18):
            self.relu3_4.add_sublayer(str(x), features[x])
        ...
        for x in range(25, 27):
            self.relu4_4.add_sublayer(str(x), features[x])
        ...
        for x in range(34, 36):
            self.relu5_4.add_sublayer(str(x), features[x])
