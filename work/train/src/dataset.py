...
from .utils import create_mask, mask_generation_with_BB, imread, random_size, imread_gray
...
class Dataset(paddle.io.Dataset):
    ...
    def load_item(self, index):
        ...
        # load image
        # 0预测图片，1擦除后的图片，2mask预测值
        line = self.data[index].strip('\n')
        path = line.split(" ")
        paths = []
        for i in path:
            i = self.dataset_dir + i
            paths.append(i)
        img = imread(paths[0])
        path_gt = paths[0].replace("JPEGImages", "gts")
        if os.path.exists(path_gt):
            img_gt = imread(path_gt)
        else:
            img_gt = imread(path_gt.split(".")[0] + '.png')
        ...
        # rr = random.random()
        # if rr < 0.5:
        #     mask_path = paths[1].replace("Annotations", "pre_segformer")
        # else:
        #     mask_path = paths[1]

        masks_pad = imread_gray(mask_path).astype(np.uint8)
        if np.max(masks_pad) < 255:
            masks_pad = (masks_pad * 255).astype(np.uint8)
        ...
        x1 = random.randint(0, W - L)
        # get left top y of crop bounding box
        y1 = random.randint(0, H - L)
        # get right bottom x of crop bounding box
        x2 = x1 + L
        # get right bottom y of crop bounding box
        y2 = y1 + L
        img = img[y1:y2, x1:x2]
        img_gt = img_gt[y1:y2, x1:x2]
        masks_pad = masks_pad[y1:y2, x1:x2]
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        masks_pad = cv2.dilate(masks_pad, kernel)  # 膨胀图像

        masks_pad_3b = cv2.cvtColor(masks_pad, cv2.COLOR_GRAY2RGB)

        img = np.where(masks_pad_3b > 0, 255, img)
        ...
        return self.to_tensor(img), self.to_tensor(img_gt), self.to_tensor((masks_pad).astype(np.float32))
    ...
    def load_mask(self, img, index):
        ...
        if mask_type == 7:
            ...
            if len(index_selected) > 0 and len(index_not_selected) > 0:
    ...
    def resize(self, img, height, width, centerCrop=True):
        ...
        img = transforms.resize(img, [height, width])
    ...
    def normalize(self, im, mean=(0, 0, 0), std=(1, 1, 1)):
