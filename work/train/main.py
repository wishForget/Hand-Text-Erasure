...
def main(mode=None):
    ...
    if paddle.device.get_device()!='cpu':
        ...
        #paddle.backends.cudnn.benchmark = True   # cudnn auto-tuner
    else:
        config.DEVICE = "cpu"
        paddle.device.set_device("cpu")

    # set cv2 running threads to 1 (prevents deadlocks with pypaddle dataloader)
    cv2.setNumThreads(0)

    # initialize random seed
    paddle.seed(config.SEED)
    #paddle.seed_all(config.SEED)
...
def test_data_loader(mode=None):
    ...
    # initialize random seed
    paddle.seed(config.SEED)
    #paddle.cuda.manual_seed_all(config.SEED)
    np.random.seed(config.SEED)
    random.seed(config.SEED)
...
def load_config(mode=None):
    ...
    parser.add_argument('--config', type=str, default='work/train/config.yml', help='model config file')
